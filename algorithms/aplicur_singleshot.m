function [xs, ts_xs, resvecs, ts_resvecs, timings, timestamps, ls, flag, matvec, pre] = ...
    aplicur_singleshot(A, mu, Amu, bmu, block_size, err_tol_, max_it, params_lsqr, verbose, ns_var)

    % ===== Defaults for optional fields in ns_var =====
    if ~isfield(ns_var, 'sketch'),       ns_var.sketch = 'SparseEmbedding'; end
    if ~isfield(ns_var, 'seed'),         ns_var.seed   = 11;                end %randi([0, 100])
    ...
    fprintf("tol_errdist = %d, tol_cvgrate = %d\n", ns_var.tol_errdist, ns_var.tol_cvgrate);
    ...
    for i = 1:max_it
        ...
        if issparse(A)
            Rr = try_chol(@() chol(R*R'), @() sketchol_qr(R', 0, 1));
        else
        ...
        if errors(i) < err_tol || i == max_it
            ...
            fprintf("minL = %.2e\t", minL);
        end
        [ys_lsqr, ~, resvec_lsqrp, ~, ts_lsqr, flag] = ...
            lsqr_scheduled(cols, apply, applyt, pre, pre, bmu, amunorm, params_lsqr, ...
